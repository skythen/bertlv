...
func TestNewBerTLV(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "constructed tlv (one child)",
		...
		{
			name:        "Error: constructed tlv with invalid child",
			inputTag:    NewOneByteTag(0x2A),
...
func TestParse(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "1B Tag, 1B Len, constructed 1C",
		...
		{
			name:       "1B Tag, 1B Len, constructed with 2C",
		...
		{
			name:       "1B Tag, 1B Len, level 2 constructed with 2C",
		...
		{
			name:        "Error: nil or empty tlv",
			inputBytes:  nil,
			expected:    nil,
			expectError: true,
		},
		{
			name:        "Error: invalid tag, wrong length",
			inputBytes:  []byte{0x7F, 0x01, 0x01},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid tag, two byte encoding indicated but not enough byte",
			inputBytes:  []byte{0x1F},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid tag, three byte encoding indicated but not enough byte",
			inputBytes:  []byte{0x1F, 0x80},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid length, empty",
			inputBytes:  []byte{0x90, 0x03, 0x01, 0x02, 0x03, 0x91},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid length, > 127 byte indicated but second byte is missing",
			inputBytes:  []byte{0x90, 0x03, 0x01, 0x02, 0x03, 0x91, 0x81},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid length, > 255 byte indicated but third byte is missing",
			inputBytes:  []byte{0x90, 0x03, 0x01, 0x02, 0x03, 0x91, 0x82, 0x00},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid length, > 255 byte indicated but two byte are missing",
			inputBytes:  []byte{0x90, 0x03, 0x01, 0x02, 0x03, 0x91, 0x82},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid length, > 127 byte but first byte does not indicate encoding with more byte ",
			inputBytes:  []byte{0x90, 0x03, 0x01, 0x02, 0x03, 0x91, 0x8F},
			expected:    BerTLVs{},
			expectError: true,
		},
		{
			name:        "Error: invalid child of constructed tlv ",
...
func TestBerTag_Class(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "application",
			tag:      NewOneByteTag(0x40),
			expected: Application,
		},
		{
			name:     "universal",
			tag:      NewTwoByteTag(0x1F, 0x7F),
			expected: Universal,
		},
		{
			name:     "context specific",
			tag:      NewOneByteTag(0x9F),
			expected: ContextSpecific,
		},
		{
			name:     "private",
			tag:      NewThreeByteTag(0xDF, 0x80, 0x90),
			expected: Private,
		},
	}
...
func TestBerTag_CheckEncoding(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "Error: invalid tag length",
			input:       []byte{0x01, 0x02, 0x03, 0x04},
			expectError: true,
		},
		{
			name:        "Error: one byte tag indicates more byte",
			input:       NewOneByteTag(0x1F),
			expectError: true,
		},
		{
			name:        "Error: two byte tag, first byte does not indicate more byte",
			input:       NewTwoByteTag(0x1E, 0x30),
			expectError: true,
		},
		{
			name:        "Error: two byte tag, second byte indicates more byte",
			input:       NewTwoByteTag(0x1F, 0x80),
			expectError: true,
		},
		{
			name:        "Error: three byte tag, first byte does not indicate more byte",
			input:       NewThreeByteTag(0x1C, 0x80, 0x10),
			expectError: true,
		},
		{
			name:        "Error: three byte tag, second byte does not indicate more byte",
...
func TestBerTLV_Bytes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "nil tag filled with zero tag",
			berTLV: BerTLV{
				Tag:   []byte{},
				Value: oneByteLenData,
			},
		...
		{
			name: "simple tag, zero inputValue",
			berTLV: BerTLV{
		...
		{
			name: "simple tag, one byte length",
			berTLV: BerTLV{
				Tag:   NewOneByteTag(0x80),
				Value: oneByteLenData,
			},
		...
		{
			name: "simple tag, two byte length",
			berTLV: BerTLV{
				Tag:   NewOneByteTag(0x10),
				Value: twoByteLenData,
			},
		...
		{
			name: "simple tag, three byte length",
			berTLV: BerTLV{
		...
		{
			name: "two byte tag, one byte length",
			berTLV: BerTLV{
				Tag:   NewTwoByteTag(0xDF, 0x20),
				Value: oneByteLenData,
			},
		...
		{
			name: "two byte tag, two byte length",
			berTLV: BerTLV{
				Tag:   NewTwoByteTag(0xDF, 0x20),
				Value: twoByteLenData,
			},
		...
		{
			name: "three byte tag, one byte length",
			berTLV: BerTLV{
		...
		{
			name: "three byte tag, two byte length",
			berTLV: BerTLV{
				Tag:   NewThreeByteTag(0xDF, 0x80, 0x20),
				Value: twoByteLenData,
			},
		...
		{
			name: "three byte tag, three byte length",
			berTLV: BerTLV{
				Tag:   NewThreeByteTag(0xDF, 0x80, 0x20),
				Value: threeByteLenData,
			},
		...
		{
			name: "three byte tag, three byte length, truncate inputValue",
			berTLV: BerTLV{
				Tag:   NewThreeByteTag(0xDF, 0x80, 0x20),
